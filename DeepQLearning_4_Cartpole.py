...
from environments.Env_Cartpole import Environment


"""
    This is the script which uses deep q learning to solve the CartPole environment, in which the 
    system must try to move a cart with an inverted pendulum on top in such a way that the pole remains upright. 
    The two actions that can be taken is applying a positive or a negative force to the cart. 
    Not giving a force to the cart is not an option
    ...
    This is the second environment for which I used deep q learning in its entirety (as opposed to the shallow
    version of deep q learning used in Temperature_static and Temperature_dynamic).
    
    Running this script will train a model on the CartPole environment and save a video every X number of 
...
class DeepAgent:
    ...
    def __init__(self):
        ...
        # For testing states
        self.test_states = [[-0.05, 0.05, -0.05, 0.05]]     # what the state is for every
    ...
    def RunDeepQLearning(self):
        ...
        plt.savefig('History_of_training_Cartpole.png')
