...
"""
    This is the static temperature environment. The goal for this environment is to reach a given temperature (38 C).
    The environment starts with a random temperature between 0 and 100 degrees celcius and can be raised or decreased
    by 1 degree every timestep. 
...
class Environment:
    def __init__(self):
        ...
        self.t_goal = 38                    # The goal we wish to achieve (in this case, 38 degrees warmth)
    ...
    def reset(self):
        ...
        :return: --
        """
        self.state = [random.randint(self.temperature_range[0], self.temperature_range[1])]       # start state
    ...
    def calculateReward(self, state):
        ...
        if abs(state[0]-self.t_goal) <= 0:
            reward = 75
        else:
            reward = 50 - abs(state[0]-self.t_goal)
        return reward
    ...
    def createRenderBG(self):
        ...
        bg = cv2.putText(bg, f"Temperature static environment", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    ...
    def render(self, state, action):
        ...
        :param state: Current state of the environment [current temperature]
        ...
        y_goal = int(self.t_goal / (self.temperature_range[1] - self.temperature_range[0]) * (self.bg_y1 - self.bg_y2) + self.bg_y2)
        bg = cv2.line(bg, (self.bg_x1 - 15, y_goal), (self.bg_x2 + 15, y_goal), (0, 0, 255), 10)
        bg = cv2.putText(bg, str(self.t_goal), (self.bg_x1 - 70, y_goal+10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
