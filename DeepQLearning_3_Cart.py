...
from environments.Env_Cart import Environment

"""
    This is the script which uses deep q learning to solve the Cart environment, in which the 
    system must try to move a cart to a random location on a line. The two actions that can be taken is applying a
    positive or a negative force to the cart. Not giving a force to the cart is not an option
    ...
    This is the first environment for which I used deep q learning in its entirety (as opposed to the shallow
    version of deep q learning used in Temperature_static and Temperature_dynamic). This environment requires 
    looking farther into the future, due to having to slow down before reaching the required destination, or risk
    overshooting.
    
    Running this script will train a model on the cart environment and save a video every X number of 
...
class DeepAgent:
    ...
    def __init__(self):
        ...
        # Memory
        self.replay_memory = deque(maxlen=2000)       # the memory we replay (First in - First out)

        # Neural network training variables
        self.train_episodes = 300       # How many training episodes we have (= a full run of the environment)
        ...
        # For testing states
        self.test_states = [[50, 0, 20], [50, 0, 30], [50, 0, 65], [50, 0, 80]]     # what the state is for every
    ...
    def test(self, episode_num):
        ...
        for test_state in self.test_states:
            ...
            self.environment.state[0] = test_state[0]              # set our test state (current x, xdot, xgoal)
            self.environment.state[1] = test_state[1]
            self.environment.state[2] = test_state[2]
    ...
    def RunDeepQLearning(self):
        ...
        plt.savefig('History_of_training_Cart.png')
